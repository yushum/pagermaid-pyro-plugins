...
用法：
  ...
  ,conflict -l <消息链接> [200|30m] 远程分析消息所在群组
...
def _parse_message_link(link: str) -> Tuple[Optional[object], Optional[int]]:
    ...
    if len(path) in (3, 4) and path[0] == "c":
        if path[1].isdigit() and all(part.isdigit() for part in path[2:]):
            return int(f"-100{path[1]}"), int(path[-1])
    if len(path) in (2, 3):
...
def _parse_args(args: list) -> Tuple[WindowSpec, Optional[str]]:
    clean = []
    link = None
    index = 0
    ...
    if len(clean) > 1:
        raise ValueError("窗口参数只能指定一个，例如 `200`、`30m` 或 `2h`。")
    if not clean:
        return WindowSpec(), link
    ...
    if value.isdigit():
        ...
        return WindowSpec(count=count), link

    seconds = _parse_duration(value)
    if seconds is None:
        raise ValueError("无法识别窗口参数，请使用 `200`、`30m`、`2h` 等格式。")
    return WindowSpec(count=None, seconds=seconds), link
...
async def _get_messages_batch(client, chat_id, message_ids: List[int]) -> list:
    if not message_ids:
        return []
    result = await client.get_messages(chat_id, message_ids)
    if result is None:
        return []
...
async def _fetch_candidate_window(
    ...
    while scanned_each_side < MAX_SCAN_IDS:
        ...
        for raw in before_raw + after_raw:
            if not raw or getattr(raw, "empty", False) or raw.id in excluded:
                continue
...
async def _include_reply_context(
    client, chat_id, records: List[ChatRecord]
) -> List[ChatRecord]:
    """补取窗口外被直接回复的原消息，避免只有反驳而没有上文。"""
    existing = {item.message_id for item in records}
    reply_ids = []
    for item in records:
        reply_id = item.reply_to_message_id
        if reply_id and reply_id not in existing and reply_id not in reply_ids:
            reply_ids.append(reply_id)
        if len(reply_ids) >= MAX_REPLY_CONTEXT:
            break
    if not reply_ids:
        return records

    raw_messages = await _get_messages_batch(client, chat_id, reply_ids)
    extra = []
    for raw in raw_messages:
        if not raw or getattr(raw, "empty", False):
            continue
        record = _message_to_record(raw)
        if record and record.message_id not in existing:
            record.context_only = True
            extra.append(record)
            existing.add(record.message_id)
    return sorted(records + extra, key=lambda item: item.message_id)
...
def _format_records(records: List[ChatRecord], anchor_id: int) -> str:
    ...
    # 极长窗口优先保留最接近锚点的消息，再恢复时间顺序。
    nearest = sorted(
        records, key=lambda item: (abs(item.message_id - anchor_id), item.message_id)
    )
    kept = []
    used = 0
    for item in nearest:
        line = _format_records([item], anchor_id)
        if used + len(line) + 1 > MAX_INPUT_CHARS:
            continue
        kept.append(item)
        used += len(line) + 1
    return _format_records(sorted(kept, key=lambda item: item.message_id), anchor_id)
...
@listener(
    ...
    description=(
        "分析群聊争执的起因、过程和各方责任\n\n"
        "回复消息：,conflict [200|30m|2h]\n"
        "消息链接：,conflict -l <链接> [200|30m|2h]\n"
        "配置：setapi / seturl / setmodel / setdisplay / showconfig"
    ),
    parameters="[消息数量|时间窗口] [-l <消息链接>] / setapi / showconfig",
)
async def conflict_analyzer(client: Client, message: Message) -> None:
    ...
    chat_title = None
    if link:
        chat_identifier, anchor_id = _parse_message_link(link)
        ...
        try:
            ...
            anchor = await client.get_messages(chat_id, anchor_id)
    ...
    window_label = (
        f"最多 {spec.count} 条" if spec.count is not None
        else f"前后各 {spec.seconds // 60} 分钟"
    )
    ...
    try:
        ...
        records = await _include_reply_context(client, chat_id, records)
    except Exception as exc:
    ...
    transcript = _format_records(records, anchor.id)
    ...
    try:
        ...
        relevant = _select_relevant_records(records, selection, anchor.id)
